...
class PageMultiBuilder extends Wrapper
{
    /**
     * The page properties.
     *
    ...
    protected function installWithTabs()
    {
        ...
        foreach ($this->sections as $section) {
            ...
            $this->installChildSection($section);
        }
    }
    ...
    protected function installWithoutTabs()
    {
        ...
        foreach ($this->sections as $section) {
            ...
            if(isset($section['args']['children'])){
                foreach($section['args']['children'] as $child_section){
                    $child_section = $child_section->getData();
                    add_settings_section($child_section['slug'], $child_section['name'], [$this, 'displaySections'], $this->datas['slug']);
                }
            }
    ...
    {
        foreach($this->sections as $section){
            $section_data = $section->getData();

            if($args['id'] === $section_data['slug'] ){
                $section->renderSections($this);
            }
        }
    }
    ...
    /**
     * Return the active child section slug of the settings page.
     *
     * @return string
     */
    protected function getActiveChildSection($section)
    {
        $firstSection = $section['args']['children'][0];


        return isset($_GET['section']) ? $_GET['section'] : $firstSection->getData()['slug'];
    }
            ...
            foreach ($this->sections as $section) {
                ...
                if ($this->getActiveTab() === $section['slug']) {
                    ...
                    if(!isset($section['args']['children'])){
                        ...
                        foreach($section['args']['children'] as $child_section){
                            $child_section = $child_section->getData();
                            if($this->getActiveChildSection($section) === $child_section['slug']){
                                settings_fields($child_section['slug']);
                                do_settings_sections($child_section['slug']);
                            }
                        }
                        $this->addSections(array_merge($this->sections,$section['args']['children']));
                    }
    ...
    private function installChildSection($section){

        if(isset($section['args']['children'])){
            foreach($section['args']['children'] as $child_section){
                $child_section = $child_section->getData();
                if (false === get_option($child_section['slug'])) {
                    add_option($child_section['slug']);
                }
                add_settings_section($child_section['slug'], $child_section['name'], [$this, 'displaySections'], $child_section['slug']);
                register_setting($child_section['slug'], $child_section['slug'], [$this, 'validateSettings']);
            }
